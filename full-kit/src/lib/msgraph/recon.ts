...
const AUTOMATED_LOCAL_PART =
  /^(noreply|no-reply|donotreply|do-not-reply|mailer|notifications?|alerts?|updates?|news|newsletter|digest|info|support|help|customercare|postmaster|bounces?|delivery|system|marketing|hello|team|admin)(\+.*)?$/i
...
export async function runSenderRecon(
  ...
  const platformBreakdown: PlatformSubjectPattern[] = [
    ...platformAggs.values(),
