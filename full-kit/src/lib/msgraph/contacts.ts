...
export function mapGraphToContact(gc: GraphContact): MappedContact {
  ...
  const createOnly: ContactCreateOnlyFields = {
    ...
    notes:
      gc.personalNotes && gc.personalNotes.length > 0 ? gc.personalNotes : null,
  }
...
export async function loadCursor(): Promise<Cursor | null> {
  const row = await db.externalSync.findUnique({
    where: {
      source_externalId: { source: SOURCE, externalId: CURSOR_EXTERNAL_ID },
    },
  })
  if (!row) return null
...
export async function syncMicrosoftContacts(): Promise<SyncResult> {
  ...
  const lockRows =
    (await db.$queryRaw`SELECT pg_try_advisory_lock(hashtext(${LOCK_KEY_LABEL})) AS got`) as {
      got: boolean
    }[]
  if (!lockRows[0]?.got) {
...
async function runSync(
  ...
  return {
    isBootstrap,
    bootstrapReason:
      internalBootstrapReason ?? (isBootstrap ? "no-cursor" : undefined),
    skippedLocked: false,
