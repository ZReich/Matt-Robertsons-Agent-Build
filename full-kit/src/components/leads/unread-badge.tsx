...
export function UnreadBadge() {
  ...
  const refresh = useCallback(() => {
    void (async () => {
      try {
        const response = await fetch("/api/vault/leads/unread-count", {
          cache: "no-store",
        })
  ...
  useEffect(() => {
    window.addEventListener("leads-unread-changed", refresh)
    return () => window.removeEventListener("leads-unread-changed", refresh)
  }, [refresh])
